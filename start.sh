...
if [ "$1" = "--dev" ]; then
    ...
    docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" --profile dev --profile tools up -d --scale worker-runner="$WORKER_CONCURRENCY"
    ...
    docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up -d --scale worker-runner="$WORKER_CONCURRENCY"
    docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" --profile bootstrap run --rm worker-bootstrap
