...
def main():
    ...
    for result in sync_fastf1_catalog(base_url=args.base_url, api_key=args.api_key, start_year=args.start_year, end_year=args.end_year, include_testing=args.include_testing):
        year = result["year"]
        print(f"{year}: events inserted={result.get('eventsInserted')} updated={result.get('eventsUpdated')} sessions inserted={result.get('sessionsInserted')} updated={result.get('sessionsUpdated')}")
